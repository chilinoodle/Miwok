...
public class WordAdapter extends ArrayAdapter <Word> {
    ...
    public WordAdapter(Context context, ArrayList<Word> words, int colorResourceId) {
        ...
        //we passed 0 to the super method on the class because we want to inflate the view ourselves
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //we inflate the view manually so we don't pass it as a parameter in the WordAdapter

        Word currentWord = getItem(position);

        // Find the TextView in the list_item.xml layout with the ID miwok_text_view
        TextView miwokTextView = (TextView) listItemView.findViewById(R.id.miwok_text_view);
        // Set that view's text as the current word in Miwok
        miwokTextView.setText(currentWord.getMiwokTranslation());

        // Find the TextView in the list_item.xml layout with the ID default_text_view
        TextView defaultTextView = (TextView) listItemView.findViewById(R.id.default_text_view);
        // Set that view's text as the current word in Default Language
